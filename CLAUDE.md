...
`## 印刻` (Imprint) closes every entry: one 24-hour **retrieval challenge** (generation effect), output as **two bare lines** — Chinese first, then English — with no scaffolding labels ("一行中文", "One English line" are spec instructions, not entry text). The challenge must be a productive action (say, write, find, do), never contemplative ("gaze", "feel"), and must be generated from the word's own imagery — generic challenges reused across words are a violation.
...
**Etymology honesty rule:** disputed or folk etymologies must be flagged ("一说 / disputed"), never invented — this extends to *components*: a plain suffix (面子's 子) must not be assigned a fabricated meaning, and motivational-myth readings (危机 = "danger + opportunity", sincere = *sine cera*) are banned from every bullet. Beyond named examples, the rule is **procedural**: root composition (*salarium* contains *sal*) is fact-level and may be stated; origin *narratives* ("soldiers paid in salt") are anecdote-level and must be hedged ("相传/一说") unless classically attested — the better the story, the more it needs the flag. Trust-critical for a dictionary product.
